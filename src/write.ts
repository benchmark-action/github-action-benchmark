...
function buildComment(benchName: string, curSuite: Benchmark, prevSuite: Benchmark, config: Config): string {
    ...
    for (const current of curSuite.benches) {
        ...
        if (prev) {
            const difference = current.value - prev.value;
            const deviation = prev.value !== 0 ? (difference / prev.value) * 100 : difference * 100;
            let statusIcon;
            if (difference === 0) {
                statusIcon = '🟰';
            } else if (
                prCommentAbsoluteThreshold !== null &&
                prCommentAbsoluteThreshold > difference &&
                prCommentAbsoluteThreshold * -1 < difference
            ) {
                statusIcon = '〰️';
            } else if (
                prCommentPercentageThreshold !== null &&
                prCommentPercentageThreshold > deviation &&
                prCommentPercentageThreshold * -1 < deviation
            ) {
                statusIcon = '〰️';
            } else if (deviation < 0) {
                statusIcon = '✅';
...
async function leaveComment(
    ...
    return await (pr?.number && commentOnPullRequest
...
async function handleComment(benchName: string, curSuite: Benchmark, prevSuite: Benchmark, config: Config) {
    ...
    if (!commentAlways && !commentOnPullRequest) {
        core.debug('Comment check was skipped because comment-always and comment-on-pull-request are disabled');
        return;
    }

    if (!githubToken) {
        throw new Error(
            "'comment-always' or 'comment-on-pull-request' inputs are set but 'github-token' input is not set",
        );
    }
    ...
    const repoMetadata = getCurrentRepoMetadata();
    const pr = github.context.payload.pull_request;
    const commitId = curSuite.commit.id;
    const commentId = `${benchName} Summary`;
    ...
    return leaveCommitComment(repoMetadata.owner.login, repoMetadata.name, commitId, body, commentId, githubToken);
}
...
async function handleAlert(benchName: string, curSuite: Benchmark, prevSuite: Benchmark, config: Config) {
    ...
    if (commentOnAlert) {
        ...
        const res = await leaveComment(curSuite.commit.id, body, `${benchName} Alert`, githubToken, true);
