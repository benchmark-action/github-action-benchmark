...
async function writeBenchmarkToGitHubPagesWithRetry(
    ...
    if (githubToken && !skipFetchGhPages && ghRepository) {
        ...
        // See https://github.com/benchmark-action/github-action-benchmark/issues/147
    ...
    // optional native results and data
    const nativeBenchmarkDataPathDst = path.join(benchmarkDataDirFullPath, 'native');
    ...
    // Adds data.js from github-action-benchmark
    ...
    // Copies and git adds/commits for data generated by a native benchmarking tool i.e cargo-criterion,
    // which produces its own raw data, plots and HTML reports
    if (nativeBenchmarkDataDirPath) {
