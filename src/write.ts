...
async function writeBenchmarkToGitHubPagesWithRetry(
    ...
    // Copies native data generated by i.e cargo-criterion, which produces its own plots on HTML reports
    if (nativeBenchmarkDataDirPath) {
        const nativeBenchmarkDataPath = path.join(benchmarkDataDirFullPath, 'native');
        await io.mkdirP(nativeBenchmarkDataPath);
        await io.cp(nativeBenchmarkDataDirPath, nativeBenchmarkDataPath, { recursive: true });
    }
