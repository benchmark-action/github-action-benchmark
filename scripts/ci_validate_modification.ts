...
function validateBenchmarkResultMod<T>(diff: Diff<T>, expectedBenchName: string, afterSuites: BenchmarkSuites) {
    ...
    if (!deepEq(diff.path, ['entries', expectedBenchName])) {
        console.log('Diff check was skipped since this diff is generated by other job. path:', diff.path);
        return;
    }
...
async function main() {
    ...
    // Length may be more than 2 when retry happens. Other jobs' changes are included in these diffs
    if (!diffs || diffs.length < 2) {
        throw new Error(`At least 2 diffs should be updated: ${diffs?.length}`);
    }
    ...
    for (const diff of diffs.slice(1)) {
        console.log('Validating diff', diff);
        validateBenchmarkResultMod(diff, expectedBenchName, afterJson.entries);
    }
