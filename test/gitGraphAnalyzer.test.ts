  ...
  describe('getBranchAncestry', () => {
    ...
    it('should handle git command failure', () => {
      ...
      expect(mockWarning).toHaveBeenCalledWith(
        expect.stringContaining('Failed to get ancestry for branch main')
      );
    });
    ...
    it('should return empty array when git CLI not available', () => {
      ...
      expect(ancestry).toEqual([]);
      expect(mockWarning).toHaveBeenCalledWith(
        'Git CLI not available, cannot determine ancestry'
      );
    });
  });
  ...
  describe('sortByGitOrderWithCLI', () => {
    ...
    it('should use git CLI when available', () => {
      ...
      expect(mockDebug).toHaveBeenCalledWith(
        'Sorted 2 benchmarks using git CLI ancestry'
      );
    });
