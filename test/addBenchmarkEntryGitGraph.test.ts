...
describe('addBenchmarkEntry with Git Graph', () => {
  ...
  it('should add to existing benchmark suite', () => {
    ...
    expect(entries[benchName][1]).toEqual(expect.objectContaining({
  ...
  it('should respect maxItems limit', () => {
    ...
    const oldEntries = [
      createMockBenchmark('old1'),
      createMockBenchmark('old2'),
      createMockBenchmark('old3'),
    ];
    const entries = {
    ...
    expect(entries[benchName][2]).toEqual(expect.objectContaining({
      commit: benchEntry.commit,
      tool: benchEntry.tool,
    }));
  ...
  it('should not truncate when under maxItems limit', () => {
    ...
    expect(mockDebug).not.toHaveBeenCalledWith(
      expect.stringContaining('was truncated to')
    );
  });
});