...
describe('writeBenchmark() github.enterprise.corp', function () {
    const savedCwd = process.cwd();
    ...
    const serverUrl = 'https://github.enterprise.corp';
    const repoUrl = `${serverUrl}/user/repo`;
    ...
    describe('with external json file', function () {
        ...
        const savedRepository = {
            private: false,
            html_url: 'https://github.enterprise.corp/user/repo',
        } as RepositoryPayloadSubset | null;
        ...
        const normalCases: Array<{
            ...
            },
            {
                ...
                error: [
                    ...
                    '| `bench_fib_20` | `25000` ns/iter (`± 20`) | `10000` ns/iter (`± 20`) | `2.50` |',
                    '',
                    'This comment was automatically generated by [workflow](https://github.enterprise.corp/user/repo/actions?query=workflow%3AWorkflow%20name) using [github-action-benchmark](https://github.com/marketplace/actions/continuous-benchmark).',
                    '',
                    'CC: @user',
                ],
            },
    ...
    describe('with gh-pages branch', function () {
        ...
        for (const t of normalCases) {
            it(t.it, async function () {
                ...
                let caughtError: Error | null = null;
                let beforeData = await loadDataJs(t.config.benchmarkDataDirPath);
                if (t.gitServerUrl) {
                    beforeData = await loadDataJs(t.config.benchmarkDataDirPath, t.gitServerUrl);
                }
                const beforeDate = Date.now();
                try {
                    await writeBenchmark(t.added, t.config);
                ...
                let data = await loadDataJs(t.config.benchmarkDataDirPath);
                if (t.gitServerUrl) {
                    data = await loadDataJs(t.config.benchmarkDataDirPath, t.gitServerUrl);
                }
                ok(data);
