...
async function alert(
    ...
    const body = buildAlertComment(alerts, benchName, curEntry, prevEntry, threshold);
    let message = body;
...
export async function writeBenchmark(bench: Benchmark, config: Config) {
    ...
    try {
        ...
        if (prevBench === null) {
            ...
            await alert(name, bench, prevBench, alertThreshold, githubToken, commentOnAlert, failOnAlert);
        }
    } finally {
